defmodule Surfboard.DriverForTest do
  ...
  setup do
    saved = Enum.map(@keys, fn k -> {k, Application.fetch_env(:surfboard, k)} end)

    on_exit(fn ->
      Enum.each(saved, fn
        {k, {:ok, v}} -> Application.put_env(:surfboard, k, v)
        {k, :error} -> Application.delete_env(:surfboard, k)
      end)
    end)

    Enum.each(@keys, &Application.delete_env(:surfboard, &1))
    :ok
  end

  describe "driver_for/1 defaults (no config)" do
    test ":default is :chrome_cdp" do
      assert Surfboard.driver_for(:default) == :chrome_cdp
    end

    test ":browser is :chrome_cdp" do
      assert Surfboard.driver_for(:browser) == :chrome_cdp
    end
    ...
    test ":headless is :lightpanda when the lightpanda package is available" do
      ...
      assert Surfboard.driver_for(:headless) == :lightpanda
    end
  end
  ...
  describe "driver_for/1 honors config overrides" do
    ...
    test ":browser follows config :browser" do
      Application.put_env(:surfboard, :browser, :chrome)
      assert Surfboard.driver_for(:browser) == :chrome
    end

    test ":headless follows config :headless, overriding the lightpanda default" do
      Application.put_env(:surfboard, :headless, :chrome_cdp)
      assert Surfboard.driver_for(:headless) == :chrome_cdp
    end
    ...
    test ":headless falls back to the :browser driver when set there" do
      ...
      Application.put_env(:surfboard, :browser, :chrome)
      Application.put_env(:surfboard, :headless, :chrome)
      assert Surfboard.driver_for(:headless) == :chrome
    end
  end
  ...
  describe "pinned_driver/0 and primary_driver/0 (SURFBOARD_DRIVER)" do
    setup do
      on_exit(fn ->
        System.delete_env("SURFBOARD_DRIVER")
        System.delete_env("SURFBOARD_BROWSER")
      end)
    end

    test "no env pin → nil / config default" do
      assert Surfboard.pinned_driver() == nil
      assert Surfboard.primary_driver() == :chrome_cdp
    end

    test "SURFBOARD_DRIVER pins to the named driver (by value)" do
      System.put_env("SURFBOARD_DRIVER", "chrome_cdp")
      assert Surfboard.pinned_driver() == :chrome_cdp
      # The pin wins over config :driver — this is what CI lanes rely on.
      Application.put_env(:surfboard, :driver, :chrome_cdp)
      assert Surfboard.primary_driver() == :chrome_cdp
    end

    test "SURFBOARD_BROWSER takes precedence over SURFBOARD_DRIVER" do
      System.put_env("SURFBOARD_DRIVER", "lightpanda")
      System.put_env("SURFBOARD_BROWSER", "chrome")
      assert Surfboard.pinned_driver() == :chrome
    end

    test "an unknown driver name raises" do
      System.put_env("SURFBOARD_DRIVER", "nope")

      assert_raise ArgumentError, ~r/not a known driver/, fn ->
        Surfboard.pinned_driver()
      end
    end
  end
end
