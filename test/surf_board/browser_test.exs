defmodule SurfBoard.BrowserTest do
  use ExUnit.Case

  import SurfBoard.SettingsTestHelpers
  ...
  describe "visit/2" do
    setup do
      ensure_setting_is_reset(:surf_board, :base_url)
      start_supervised!(TestDriver)
      :ok
    end

    test "relative path without leading slash, base url with trailing slash" do
      Application.put_env(:surf_board, :base_url, "http://example.com/")
    ...
    test "relative path with leading slash, base url ending in /api" do
      Application.put_env(:surf_board, :base_url, "https://example.com:9090/api/")
    ...
    test "relative url when the base_url isn't configured" do
      Application.delete_env(:surf_board, :base_url)
    ...
    test "absolute url when the base_url is configured" do
      Application.put_env(:surf_board, :base_url, "http://example.org:5555/test")
      uri = "https://example.com:9090/api/form.html"
  ...
  describe "grant_permissions/2" do
    ...
    test "raises on a driver that doesn't support it (e.g. Lightpanda)" do
      session = %Session{
        spec_module: SurfBoard.SpecModule.LightpandaCDP,
        spec: SurfBoard.SpecModule.LightpandaCDP.spec()
      }
  ...
  defp session_for_driver(driver) do
    %Session{spec_module: driver, spec: driver.spec()}
  end
end
