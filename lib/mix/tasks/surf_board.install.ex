defmodule Mix.Tasks.SurfBoard.Install do
  @moduledoc """
  ...
  ## Usage
      ...
      mix surf_board.install              # all browsers, latest stable Chrome
      mix surf_board.install 147.0.7727   # all browsers, specific Chrome version

  To install just one browser, use the scoped subtasks:

      mix surf_board.install.chrome       # Chrome for Testing + chromium-bidi deps
      mix surf_board.install.lightpanda   # Lightpanda only
  ...
  def run(args) do
    ...
    SurfBoard.Installer.install_all(version)
  end
end
