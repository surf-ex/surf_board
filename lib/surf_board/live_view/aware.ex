defmodule SurfBoard.LiveView.Aware do
  ...
  #
  # All page-side logic lives in priv/surf_board.js as `W.preparePatch`,
  ...
  Used by `SurfBoard.Browser.await_patch/2` via the driver behaviour.
  """
  ...
  defp warn_no_livesocket_once(%Session{id: id}) do
    ...
    unless Process.get(key) do
      ...
      Logger.warning("""
      [surf_board] LiveView page served, but window.liveSocket never \
      ...
      or run `MIX_ENV=test mix assets.build` before the suite.\
      """)
    end
