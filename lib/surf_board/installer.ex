defmodule SurfBoard.Installer do
  @moduledoc false

  # Shared implementation behind the `mix surf_board.install[.*]` tasks.
  #
    ...
    case BrowserPaths.system_chrome() do
      ...
      nil ->
        if BrowserPaths.chrome_for_testing_unsupported?() do
          ...
          surf_board looks for `google-chrome`, `chromium`, or
          `chromium-browser` on PATH. You can also point it at a binary
          directly with SURF_BOARD_CHROME_PATH, or at a remote browser with
          SURF_BOARD_CHROME_URL.
          """)
        end
  ...
  defp supports_install_dir? do
    sentinel =
      Path.join(System.tmp_dir!(), "surf_board-lp-probe-#{System.unique_integer([:positive])}")
  ...
  defp install_bidi_server_deps do
    ...
    cond do
      not File.dir?(bidi_dir) ->
        ...
        This usually means `surf_board` was loaded but its package files
        weren't unpacked, or the task is being run from outside a Mix
        project. Try `mix deps.get && mix deps.compile surf_board` first.
        """)
  ...
  # The chromium-bidi server lives in surf_board's `priv/bidi-server/`
  # — when surf_board is a dependency we have to resolve through
  # Application.app_dir/2, not the consumer's File.cwd!. Falls back to
  # cwd-relative for in-tree development (where Mix.Project is surf_board
  # itself).
  defp bidi_server_dir do
    # Application.app_dir/2 returns a path string (and raises ArgumentError
    # if :surf_board isn't loaded — caught below). Use it when it points at a
    # real dir, otherwise fall back to cwd-relative for in-tree dev.
    path = Application.app_dir(:surf_board, "priv/bidi-server")
  ...
  # Application.app_dir/2 (used to find priv/bidi-server when surf_board
  ...
  defp load_app!, do: Application.load(:surf_board)

  defp ensure_npx! do
    unless System.find_executable("npx") do
      Mix.raise("""
      `npx` not found. Install Node.js to use `mix surf_board.install`.

      Alternatively, set the path manually:
        SURF_BOARD_CHROME_PATH=/path/to/chrome
      """)
    end
  end
  ...
  defp ensure_npm! do
    unless System.find_executable("npm") do
      Mix.raise("""
      `npm` not found. Install Node.js to use `mix surf_board.install`.
