defmodule SurfBoard.Driver.Generic do
  @moduledoc false

  # The fully data-driven Driver. Implements every SurfBoard.Driver
  ...
  # this driver. After it returns, the session is "generic" — SurfBoard
  ...
  alias SurfBoard.Driver.{Orchestrator, Spec}

  # ----- The __using__ macro -----
  #
  # Concrete drivers `use SurfBoard.Driver.Generic` to inherit
  ...
  defmacro __using__(_opts) do
    delegates =
      for {name, arity, is_callback} <- SurfBoard.Driver.Generic.__generic_callbacks__() do
        args = Macro.generate_arguments(arity, __MODULE__)

        impl_attr =
          if is_callback do
            quote do: @impl(SurfBoard.Driver)
          end

        quote do
          unquote(impl_attr)
          defdelegate unquote(name)(unquote_splicing(args)), to: SurfBoard.Driver.Generic
        end
      end

    overridable =
      for {name, arity, _is_callback} <- SurfBoard.Driver.Generic.__generic_callbacks__() do
        {name, arity}
      end
