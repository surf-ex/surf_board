defmodule SurfBoard.Driver.Orchestrator do
  @moduledoc false
  ...
  import SurfBoard.Driver.LogChecker
  ...
  Protocol-agnostic (uses SurfBoard.Protocol.eval_async under the
  hood).
  """
  ...
  the driver opts in via `log_check_interactions?`. The LiveView-connect
  await only runs when the session opted in via
  `start_session(live_view_aware: true)` — a plain scraping/automation
  session skips it entirely rather than paying a Phoenix-detection
  probe on every navigation.
  """
  ...
  def visit(%Spec{} = spec, %Session{} = session, url) do
    flow = fn ->
      ...
      if session.live_view_aware?, do: _ = LiveViewAware.await_liveview_connected(session)
      result
    end
  ...
  @doc """
  Click an element. Consults the session's `live_view_aware?` flag (via
  `click_strategy_for/1`) for the flow shape, wraps in log-check when
  `spec.log_check_interactions?`. Patch-classified timeouts fall
  through silently — the bootstrap's `nav_pending` signal extends the
  page_ready wait when an LV redirect is in flight, so no caller
  polling is needed.
  """
  ...
  Used by `SurfBoard.Browser.click(query, await: :defer)` for tests
  that need to observe optimistic-UI DOM state between the click and
  the server reply.

  Falls back to a plain click (no classify, no wait) when the session
  isn't `live_view_aware?` — there's no awaiting machinery to skip, so
  `:defer` collapses to the normal path.
  """
  @spec click_deferred(Spec.t(), Element.t()) :: {:ok, String.t() | nil} | {:error, term}
  def click_deferred(%Spec{} = spec, %Element{} = element) do
    session = Element.root_session(element)

    case click_strategy_for(session) do
      :simple ->
  ...
  string-keyed map the `SurfBoard.Driver` contract expects.
  """
  ...
  def find_elements(%Spec{} = spec, parent, query) do
    %SurfBoard.Query{} = q = ensure_query(query)
  ...
  defp ensure_query(%SurfBoard.Query{} = q), do: q

  defp ensure_query({type, selector}) when type in [:css, :xpath] and is_binary(selector) do
    SurfBoard.Query.css(selector)
  end

  defp maybe_log_check(true, session, flow), do: check_logs!(session, flow)
  defp maybe_log_check(false, _session, flow), do: flow.()

  # LiveView `phx-*` classification is a session-level opt-in
  # (`start_session(live_view_aware: true)`), off by default for every
  # driver — a plain scraping/automation session pays no cost for a
  # Phoenix concept it doesn't use. `:simple` here matches what every
  # driver already does when live_view_aware? is false.
  defp click_strategy_for(%SurfBoard.Session{live_view_aware?: true}), do: :classified
  defp click_strategy_for(%SurfBoard.Session{}), do: :simple

  defp do_click(%Spec{} = spec, session, element) do
    case click_strategy_for(session) do
      :simple ->
      ...
      :classified ->
        case spec.wire_protocol.click_aware_with_classification(session, element) do
          ...
          {:ok, _classification, :timeout} ->
            ...
            # `live_redirect`/`redirect` payload), so callers' own
            # find/has? retries take it from here without us polling
            # current_url.
            {:ok, nil}
