defmodule SurfBoard.Launcher.BrowserPaths do
  ...
  1. `SURF_BOARD_CHROME_URL` — connect to remote Chrome (`chrome:9222` or full `ws://` URL)
  2. `SURF_BOARD_CHROME_PATH` — local Chrome binary to launch
  3. `.browsers/PATHS` file (written by `mix surf_board.install`)
  4. System PATH

  ## Lightpanda resolution order

  1. `SURF_BOARD_LIGHTPANDA_PATH` — local Lightpanda binary to launch
  2. `.browsers/PATHS` file (written by `mix surf_board.install`)
  ...
  Or for Docker/CI:
      ...
      SURF_BOARD_CHROME_URL=chrome:9222 mix test.chrome
      SURF_BOARD_LIGHTPANDA_PATH=/opt/lightpanda/lightpanda mix test.lightpanda
  """
  ...
  """
  def chrome do
    with :skip <- from_url("SURF_BOARD_CHROME_URL"),
         :skip <- from_path("SURF_BOARD_CHROME_PATH"),
  ...
  arm64/aarch64 Linux. On these, `mix surf_board.install` can't download
  ...
  def chrome_path! do
    case chrome() do
      ...
      {:url, _} ->
        raise "SURF_BOARD_CHROME_URL is set — use chrome_url/0 instead of chrome_path!/0"

      :error ->
        raise "Chrome not found. Run `mix surf_board.install` or set SURF_BOARD_CHROME_PATH."
    end
  end
  ...
  """
  def lightpanda do
    with :skip <- from_path("SURF_BOARD_LIGHTPANDA_PATH"),
  ...
  Used both by `mix surf_board.install` (to place the binary) and by the
