defmodule SurfBoard.OpsShared do
  ...
  @xpath_polyfill_path Path.join(:code.priv_dir(:surf_board), "cdp/wgxpath.install.js")
  ...
  # priv/surf_board.js as the W.run interpreter (W.text, W.attribute,
  ...
  defmacro __using__(_opts) do
    ...
    quote do
      ...
      priv/surf_board.js — returns one of `"patch"`, `"navigate"`,
      `"full_page"`, or `"none"`.
      """
      ...
      """
      @spec find_elements(Session.t() | Element.t(), SurfBoard.Query.t(), keyword) ::
              {:ok, [Element.t()]} | {:error, term}
      def find_elements(parent, %SurfBoard.Query{} = query, opts \\ []) do
      ...
      """
      @spec find_elements_lazy(Session.t() | Element.t(), SurfBoard.Query.t(), keyword) ::
              {:ok, [Element.t()]} | {:error, term}
      def find_elements_lazy(parent, %SurfBoard.Query{} = query, opts \\ []) do
  ...
  @spec do_find_elements(
          ...
          {:ok, [Element.t()]} | {:error, term}
  def do_find_elements(client, parent, %SurfBoard.Query{} = query, opts, mode) do
    session = Element.root_session(parent)
    timeout = Keyword.get(opts, :timeout, 5_000)
    count = SurfBoard.Query.count(query)
