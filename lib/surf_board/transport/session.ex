defmodule SurfBoard.Transport.Session do
  @moduledoc false

  # Per-session coordinator. Owns:
  #
  #   * the session struct (`%SurfBoard.Session{}` with caps/url/etc.)
  ...
  Opts:
    * `:ws_pid` (required)
    * `:init_fun` — 0-arity function returning `{:ok, %SurfBoard.Session{}}`
  ...
  """
  @spec cdp_send(SurfBoard.Session.t(), String.t(), map, keyword) ::
          {:ok, term} | {:error, term}
  def cdp_send(%SurfBoard.Session{pid: pid}, method, params, opts \\ [])
      when is_pid(pid) do
  ...
  """
  @spec cdp_cast(SurfBoard.Session.t(), String.t(), map, keyword) :: :ok
  def cdp_cast(%SurfBoard.Session{pid: pid}, method, params, opts \\ [])
      when is_pid(pid) do
  ...
  """
  @spec subscribe(SurfBoard.Session.t(), String.t(), :global | nil) :: :ok
  def subscribe(%SurfBoard.Session{} = session, event_method, routing_key \\ nil)
  ...
  """
  @spec await_page_load(SurfBoard.Session.t(), String.t(), String.t(), timeout) ::
          :ok | :timeout
  def await_page_load(%SurfBoard.Session{pid: pid}, loader_id, name, timeout_ms \\ 10_000)
  ...
  """
  @spec await_next_page_load(SurfBoard.Session.t(), String.t(), timeout) :: :ok | :timeout
  def await_next_page_load(%SurfBoard.Session{pid: pid}, name \\ "load", timeout_ms \\ 10_000)
  ...
  """
  @spec sync_barrier(SurfBoard.Session.t()) :: :ok
  def sync_barrier(%SurfBoard.Session{pid: pid}) when is_pid(pid) do
    GenServer.call(pid, :sync_barrier)
  catch
    :exit, _ -> :ok
  end

  def sync_barrier(%SurfBoard.Session{}), do: :ok
  ...
  def register_find(%SurfBoard.Session{pid: pid}, query_id, timeout_ms)
  ...
  def await_find_result(%SurfBoard.Session{pid: pid}, query_id, timeout_ms)
      when is_pid(pid) and is_binary(query_id) do
  ...
  """
  @spec await_page_ready_after(SurfBoard.Session.t(), String.t() | nil, timeout) ::
          :ok | :timeout
  def await_page_ready_after(%SurfBoard.Session{pid: pid}, pre_page_id, timeout_ms \\ 5_000)
      when is_pid(pid) do
  ...
  """
  @spec current_context_id(SurfBoard.Session.t()) :: integer | nil
  def current_context_id(%SurfBoard.Session{pid: pid}) when is_pid(pid) do
  ...
  """
  @spec get_page_id(SurfBoard.Session.t()) :: String.t() | nil
  def get_page_id(%SurfBoard.Session{pid: pid}) when is_pid(pid) do
  ...
  before calling this.
  """
  @spec push_frame(SurfBoard.Session.t(), integer) :: :ok
  def push_frame(%SurfBoard.Session{pid: pid}, context_id)
  ...
  @spec pop_frame(SurfBoard.Session.t()) :: :ok
  def pop_frame(%SurfBoard.Session{pid: pid}) when is_pid(pid) do
  ...
  """
  @spec record_frame_context(SurfBoard.Session.t(), String.t(), integer) :: :ok
  def record_frame_context(%SurfBoard.Session{pid: pid}, frame_id, context_id)
  ...
  """
  @spec lookup_frame_context(SurfBoard.Session.t(), String.t()) :: integer | nil
  def lookup_frame_context(%SurfBoard.Session{pid: pid}, frame_id)
  ...
  """
  def stop(%SurfBoard.Session{pid: pid}) when is_pid(pid) do
  ...
  def init({ws_pid, init_fun, teardown_fun, owner}) do
    ...
    case init_fun.() do
      {:ok, %SurfBoard.Session{} = session} ->
        ...
        # Register with SessionStore so SurfBoard.Feature can discover
