defmodule SurfBoard.CDP.Ops do
  @moduledoc false
  ...
  #
  # ## Opcodes (the W.run vocabulary — see priv/surf_board.js)
  #
  ...
  @doc """
  Apply filters from a `SurfBoard.Query` to an in-progress ops pipeline.
  """
  def from_query(%__MODULE__{} = o, query) do
    o =
      case SurfBoard.Query.visible?(query) do
        true -> visible(o, true)
    ...
    o =
      case SurfBoard.Query.inner_text(query) do
        nil -> o
        t -> text(o, t)
      end

    case SurfBoard.Query.selected?(query) do
  ...
  @doc """
  Build a complete opcode sequence from a `SurfBoard.Query`.
  Validates and compiles the query and applies all filters.

  Returns `{:ok, ops, query}` or `{:error, reason}`.
  """
  def compile_query(parent, %SurfBoard.Query{} = query) do
    with {:ok, validated} <- SurfBoard.Query.validate(query) do
      {type, selector} = SurfBoard.Query.compile(validated)
