defmodule SurfBoard.Bench.Timing do
  ...
  # SURF_BOARD_BENCH_TIMING env var is set. When disabled, every
  ...
  @outliers :surf_board_bench_outliers
  @outlier_threshold_ns 1_000_000_000

  def enabled?, do: System.get_env("SURF_BOARD_BENCH_TIMING") in ["1", "true"]
  ...
  def report do
    if enabled?() do
      ...
      IO.puts(
        :stderr,
        "[surf_board-bench] CDP gap: #{count} calls, total=#{total_ms}ms, " <>
      ...
      |> Enum.each(fn {m, t} ->
        ...
        IO.puts(
          :stderr,
          "[surf_board-bench]   #{String.pad_trailing(m, 42)} " <>
      ...
      if outliers != [] do
        IO.puts(:stderr, "[surf_board-bench] long-tail (>1s) by method:")
