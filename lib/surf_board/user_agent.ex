defmodule SurfBoard.UserAgent do
  ...
  # first):
  #
  #   1. the `:user_agent` option passed to `SurfBoard.start_session/1`
  #   2. `config :surf_board, user_agent: "..."`
  ...
  # on that binary shares one UA. `SurfBoard.start_session(user_agent:)`
  ...
  def configured, do: Application.get_env(:surf_board, @config_key)
