defmodule Surfboard.Driver do
  ...
  alias Surfboard.{Element, Query, Session}
  ...
  Every driver (CDP, BiDi, Lightpanda) implements this by arming a
  JS-side promise on `liveSocket.main.domCallbacks.onPatchEnd` and
  racing it against the configured timeout. A no-op on sessions that
  didn't opt in via `live_view_aware: true` — see
  `Surfboard.start_session/1`.

  See `Surfboard.Browser.await_patch/2` for the public-facing
  contract.
  """
  ...
  drivers without support raise `Surfboard.DriverError`.
  """
