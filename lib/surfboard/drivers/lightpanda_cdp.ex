defmodule Surfboard.Drivers.LightpandaCDP do
  ...
  # Lightpanda driver speaking CDP over the surfboard transport stack.
  ...
  # `config :surfboard, user_agent: "..."` is the cross-driver setting; it
  ...
  defp user_agent_args do
    ...
    suffix = Application.get_env(:surfboard, :lightpanda_user_agent_suffix)
  ...
  # warns rather than appearing to work. `config :surfboard, user_agent:`
  ...
  defp warn_user_agent_unsupported do
    unless :persistent_term.get(@warned_ua_key, false) do
      ...
      Logger.warning("""
      [surfboard] the :user_agent session option has no effect on the \
      ...
      for the whole browser instead — this works on every driver:
          ...
          config :surfboard, user_agent: "MyScraper/1.0 (+https://…)"

          # or append to Lightpanda/X.Y rather than replacing it:
          config :surfboard, lightpanda_user_agent_suffix: "MyScraper/1.0"
