defmodule Surfboard.Query do
  ...
  a bare query retries like any other, checking absence without it
  otherwise asserts "never appears within the window" — true and
  useful for "this content must not leak", but wrong for "this hasn't
  happened *yet*":

  ```
  # optimistic UI: the client-side update is in, the server reply is not
  true = Browser.has?(session, Query.css("#fast", text: "Ontario"))
  false = Browser.has?(session, Query.css("#slow", text: "Ontario", wait: 0))
  ```

  Without `wait: 0` that check would wait out the round-trip, see the
  legitimate update land, and return `true`.
  ...
  `Surfboard.Query.compile/1` - it takes a query and returns the CSS or xpath
  query that will be sent to the driver:

      iex> Surfboard.Query.compile Surfboard.Query.text("my text")
  ...
  To check text that appears in multiple (potentially nested) elements,
  rather than using `Browser.has?/2` in combination with `Query.text/2`, use
  `Browser.has_text?/{2,3}` directly.
  ...
  ### Checking the text of a single element

      ```
      submit_button = Query.css("#submit-button")
      true = Browser.has?(session, Query.text(submit_button, "Create"))
      ```

  ### Checking the text of nested elements
  ...
  Code:
      ...
      ```
      true = Browser.has_text?(session, "Unread messages: 1 message")
      ...
      true = Browser.has?(session, Query.text(notifications_block, "Unread messages:"))
      ```
  """
  ...
  @doc """
  See `Surfboard.Query.fillable_field/2`.
  """
  ...
  Compiles a query into CSS or xpath so its ready to be sent to the driver
      ...
      iex> Surfboard.Query.compile Surfboard.Query.text("my text")
      {:xpath, ".//*[contains(normalize-space(text()), \\"my text\\")]"}
      iex> Surfboard.Query.compile Surfboard.Query.css("#some-id")
      {:css, "#some-id"}
  """
  ...
  form that makes a negative `Browser.has?/2` check mean *absent at this
  instant* rather than *never appears within the window*.
