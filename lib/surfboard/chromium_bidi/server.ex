defmodule Surfboard.ChromiumBiDi.Server do
  @moduledoc false

  # Manages a chromium-bidi Node process that exposes a BiDi WebSocket
  # server for the Surfboard.Chrome (BiDi) driver path.
  #
  ...
  def handle_info({port, {:exit_status, status}}, %{port: port} = state) do
    Logger.error("Surfboard BiDi server exited with status #{status}; buffer: #{state.buffer}")
  ...
  def handle_info(:startup_timeout, %{ready?: false} = state) do
    Logger.error("Surfboard BiDi server didn't become ready within #{@startup_timeout}ms")
  ...
  defp parse_output(state, data) do
    ...
    if String.contains?(buffer, "surfboard-bidi-server: ready on port=") do
      case Regex.run(~r/surfboard-bidi-server: ready on port=(\d+)/, buffer) do
        [_, p] ->
  ...
  defp wrapper_script do
    Path.absname("priv/run_command.sh", Application.app_dir(:surfboard))
  end

  defp run_script do
    Path.absname("priv/bidi-server/run.mjs", Application.app_dir(:surfboard))
  end
