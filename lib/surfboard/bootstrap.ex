defmodule Surfboard.Bootstrap do
  ...
  #
  # Source of truth lives in priv/surfboard.js. We read it at compile time
  ...
  # `mix surfboard.minify`). Falls back to the readable source so dev
  # edits to surfboard.js take effect without a rebuild step. Both
  # paths are listed as @external_resource so Mix recompiles on
  # either change.
  @bootstrap_min_path Path.join([__DIR__, "..", "..", "priv", "surfboard.min.js"])
  @bootstrap_src_path Path.join([__DIR__, "..", "..", "priv", "surfboard.js"])
  ...
  def register_js(query_id, ops_json, count_js, root_js \\ "null") do
    ...
    # which lives in priv/surfboard.js.
