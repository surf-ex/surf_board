defmodule Surfboard.Bench.Timing do
  ...
  # SURFBOARD_BENCH_TIMING env var is set. When disabled, every
  ...
  @table :surfboard_bench_timing
  @outliers :surfboard_bench_outliers
  @outlier_threshold_ns 1_000_000_000

  def enabled?, do: System.get_env("SURFBOARD_BENCH_TIMING") in ["1", "true"]
  ...
  def report do
    if enabled?() do
      ...
      IO.puts(
        :stderr,
        "[surfboard-bench] CDP gap: #{count} calls, total=#{total_ms}ms, " <>
      ...
      |> Enum.each(fn {m, t} ->
        ...
        IO.puts(
          :stderr,
          "[surfboard-bench]   #{String.pad_trailing(m, 42)} " <>
      ...
      if outliers != [] do
        IO.puts(:stderr, "[surfboard-bench] long-tail (>1s) by method:")
