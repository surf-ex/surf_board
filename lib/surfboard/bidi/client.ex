defmodule Surfboard.BiDi.Client do
  ...
  # etc. Callers (drivers, tests) work in Surfboard's normalized shape
  ...
  defp ctx(%Session{id: id, browsing_context: root}) do
    case Process.get({:surfboard_bidi_v2_frame, id}) do
      nil -> root
  ...
  def click_aware_with_classification(%Session{} = session, %Element{} = element, opts \\ []) do
    ...
    with {:ok, %{"classification" => classification, "prePageId" => pre_page_id}} <-
      ...
      cond do
        ...
        await? == false ->
          # Caller stashed pre_page_id via `:pre_page_id_sink`; the
          # session-level await_patch/2 will drain the page_ready wait.
