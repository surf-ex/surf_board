defmodule Surfboard.BrowserPaths do
  ...
  1. `SURFBOARD_CHROME_URL` — connect to remote Chrome (`chrome:9222` or full `ws://` URL)
  2. `SURFBOARD_CHROME_PATH` — local Chrome binary to launch
  3. `.browsers/PATHS` file (written by `mix surfboard.install`)
  4. System PATH

  ## Lightpanda resolution order

  1. `SURFBOARD_LIGHTPANDA_PATH` — local Lightpanda binary to launch
  2. `.browsers/PATHS` file (written by `mix surfboard.install`)
  ...
  Or for Docker/CI:
      ...
      SURFBOARD_CHROME_URL=chrome:9222 mix test.chrome
      SURFBOARD_LIGHTPANDA_PATH=/opt/lightpanda/lightpanda mix test.lightpanda
  """
  ...
  """
  def chrome do
    with :skip <- from_url("SURFBOARD_CHROME_URL"),
         :skip <- from_path("SURFBOARD_CHROME_PATH"),
  ...
  arm64/aarch64 Linux. On these, `mix surfboard.install` can't download
  ...
  def chrome_path! do
    case chrome() do
      ...
      {:url, _} ->
        raise "SURFBOARD_CHROME_URL is set — use chrome_url/0 instead of chrome_path!/0"

      :error ->
        raise "Chrome not found. Run `mix surfboard.install` or set SURFBOARD_CHROME_PATH."
    end
  end
  ...
  """
  def lightpanda do
    with :skip <- from_path("SURFBOARD_LIGHTPANDA_PATH"),
  ...
  Used both by `mix surfboard.install` (to place the binary) and by the
