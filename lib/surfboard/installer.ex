defmodule Surfboard.Installer do
  ...
  # Shared implementation behind the `mix surfboard.install[.*]` tasks.
  #
    ...
    case BrowserPaths.system_chrome() do
      ...
      nil ->
        if BrowserPaths.chrome_for_testing_unsupported?() do
          ...
          surfboard looks for `google-chrome`, `chromium`, or
          `chromium-browser` on PATH. You can also point it at a binary
          directly with SURFBOARD_CHROME_PATH, or at a remote browser with
          SURFBOARD_CHROME_URL.
          """)
        end
  ...
  defp supports_install_dir? do
    sentinel =
      Path.join(System.tmp_dir!(), "surfboard-lp-probe-#{System.unique_integer([:positive])}")
  ...
  defp install_bidi_server_deps do
    ...
    cond do
      not File.dir?(bidi_dir) ->
        ...
        This usually means `surfboard` was loaded but its package files
        weren't unpacked, or the task is being run from outside a Mix
        project. Try `mix deps.get && mix deps.compile surfboard` first.
        """)
  ...
  # The chromium-bidi server lives in surfboard's `priv/bidi-server/`
  # — when surfboard is a dependency we have to resolve through
  # Application.app_dir/2, not the consumer's File.cwd!. Falls back to
  # cwd-relative for in-tree development (where Mix.Project is surfboard
  # itself).
  defp bidi_server_dir do
    # Application.app_dir/2 returns a path string (and raises ArgumentError
    # if :surfboard isn't loaded — caught below). Use it when it points at a
    # real dir, otherwise fall back to cwd-relative for in-tree dev.
    path = Application.app_dir(:surfboard, "priv/bidi-server")
  ...
  # Application.app_dir/2 (used to find priv/bidi-server when surfboard
  ...
  defp load_app!, do: Application.load(:surfboard)

  defp ensure_npx! do
    unless System.find_executable("npx") do
      Mix.raise("""
      `npx` not found. Install Node.js to use `mix surfboard.install`.

      Alternatively, set the path manually:
        SURFBOARD_CHROME_PATH=/path/to/chrome
      """)
    end
  end
  ...
  defp ensure_npm! do
    unless System.find_executable("npm") do
      Mix.raise("""
      `npm` not found. Install Node.js to use `mix surfboard.install`.
