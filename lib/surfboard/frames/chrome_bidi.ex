defmodule Surfboard.Frames.ChromeBiDi do
  ...
  def focus_frame(%Session{} = session, %Element{} = iframe) do
    case BiDiClient.child_context_for_iframe(session, iframe) do
      {:ok, child_ctx} ->
        ...
        stack = Process.get({:surfboard_bidi_v2_frame_stack, session.id}, [])
        current = current_ctx(session)
        Process.put({:surfboard_bidi_v2_frame_stack, session.id}, [current | stack])
        Process.put({:surfboard_bidi_v2_frame, session.id}, child_ctx)
        ...
        Process.put({:surfboard_frame_context, session.id}, child_ctx)
        {:ok, nil}
  ...
  def focus_frame(%Session{} = session, nil) do
    ...
    Process.delete({:surfboard_frame_context, session.id})
    {:ok, nil}
  end
  ...
  def focus_parent_frame(%Session{} = session) do
    case Process.get({:surfboard_bidi_v2_frame_stack, session.id}, []) do
      [] ->
        # Already at root.
        {:ok, nil}

      [parent_ctx | rest] ->
        Process.put({:surfboard_bidi_v2_frame_stack, session.id}, rest)

        if rest == [] and parent_ctx == session.browsing_context do
          Process.delete({:surfboard_bidi_v2_frame, session.id})
          Process.delete({:surfboard_frame_context, session.id})
        else
          Process.put({:surfboard_bidi_v2_frame, session.id}, parent_ctx)
          Process.put({:surfboard_frame_context, session.id}, parent_ctx)
        end
  ...
  defp current_ctx(%Session{id: id, browsing_context: root}) do
    Process.get({:surfboard_bidi_v2_frame, id}, root)
  end
end
