defmodule Surfboard.Browser do
  ...
  state. However, if this error does continue to occur it will cause surfboard to
  ...
  * `:await` — controls the LiveView patch-await behaviour for clicks
  ...
  defp in_frame?(%Session{} = session) do
    Process.get({:cdp_frame_stack, session.id}, []) != [] or
      Process.get({:surfboard_frame_context, session.id}) != nil
  end

  defp in_switched_window?(%Session{} = session) do
    Process.get({:cdp_current_target, session.id}) != nil or
      Process.get({:surfboard_focused_context, session.id}) != nil
  end
  ...
  defp screenshot_dir do
    Application.get_env(:surfboard, :screenshot_dir, "#{File.cwd!()}/screenshots")
  end
  ...
  * `:timeout` — max wait in ms (default: 5_000)
  ...
  # `W.classify` in priv/surfboard.js — the page-side interpreter
