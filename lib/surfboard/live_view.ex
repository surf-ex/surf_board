defmodule Surfboard.LiveView do
  ...
  *during* a server round-trip — not just after it completes.
  ...
  defp warn_no_livesocket(fun) do
    unless :persistent_term.get(@no_livesocket_key, false) do
      ...
      Logger.warning("""
      [surfboard] Surfboard.LiveView.#{fun}/2 had no effect — `window.liveSocket` \
  ...
  * `:timeout` — max wait in ms (default: `5_000`).
  ...
  defp remote?(%Session{driver: Surfboard.Drivers.ChromeBiDi}), do: true
  defp remote?(%Session{driver: Surfboard.Drivers.ChromeCDP}), do: true
  defp remote?(%Session{driver: Surfboard.Drivers.LightpandaCDP}), do: true
