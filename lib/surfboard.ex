defmodule Surfboard do
  ...
  * `:js_logger` - IO device where JavaScript console logs are written to. Defaults to :stdio. This option can also be set to a file or any other io device. You can disable JavaScript console logging by setting this to `nil`.
  ...
  ```
  config :surfboard, user_agent: "MyScraper/1.0 (+https://example.com/bot)"
  ```
  ...
  *different* User-Agents at the same time (mobile vs desktop, say):
  ...
  `config :surfboard, lightpanda_user_agent_suffix: "MyScraper/1.0"`, which
  ...
  def screenshot_on_failure? do
    Application.get_env(:surfboard, :screenshot_on_failure)
  end

  @doc false
  def js_errors? do
    Application.get_env(:surfboard, :js_errors, true)
  end

  @doc false
  def js_logger do
    Application.get_env(:surfboard, :js_logger, :stdio)
  end
end
