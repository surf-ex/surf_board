// Browser-side bootstrap for surfboard push-based element finding.
//
...
//
// References `__surfboard` as a free variable — the caller wraps this
// body in either:
//   - CDP: `(function(){ ... })()` IIFE; __surfboard is a global binding
//          installed via Runtime.addBinding.
//   - BiDi: `(__surfboard) => { ... }` arrow; __surfboard is the channel
//          callback passed in via script.addPreloadScript.
//
// Source of truth: this file. Surfboard.Bootstrap embeds it at compile
...
function send(payload) {
  try { __surfboard(JSON.stringify(payload)); } catch (e) {}
}
...
function installPatchEndDispatcher() {
  ...
  if (ls.domCallbacks.__surfboardDispatcher) return true;
  ...
  dispatcher.__surfboardDispatcher = true;
...
// --- Element operations ---
// These are the JS primitives behind Surfboard's per-element API. The
...
W.STALE = {__surfboard_stale: true};
...
// lib/surfboard/remote/live_view_aware.ex. Keeping them here means
...
// Install a one-shot promise on window.__surfboard_patch_promise that
...
W.preparePatch = function() {
  ...
  if (!window.__surfboard_patch_hooked) {
    wrapPatchEnd(function() {
      if (window.__surfboard_patch_resolve) {
        var r = window.__surfboard_patch_resolve;
        window.__surfboard_patch_resolve = null;
        r(true);
      }
    });
    ...
    window.__surfboard_patch_hooked = true;
  }
  window.__surfboard_patch_promise = new Promise(function(resolve) {
    window.__surfboard_patch_resolve = resolve;
...
W.awaitPatch = function(timeoutMs) {
  if (!window.__surfboard_patch_promise) return Promise.resolve('no-promise');
  var p = window.__surfboard_patch_promise;
  window.__surfboard_patch_promise = null;
  return Promise.race([
    p,
...
W.getWindowSize = function() {
  return JSON.stringify(window.__surfboard_window_size ||
    {width: window.innerWidth, height: window.innerHeight});
};
W.setWindowSize = function(w, h) {
  window.__surfboard_window_size = {width: w, height: h};
};
...
function installLvHook() {
  ...
  try {
    ...
    if (ls && ls.domCallbacks) {
      ...
      var sock = ls.socket;
      if (sock && typeof sock.decode === 'function' && !sock.__surfboardDecodeHooked) {
        ...
        sock.__surfboardDecodeHooked = true;
      }
      W.lvHooked = true;
